# vn/build.ps1 — 越南宪制历史构建脚本
# 用法: .\vn\build.ps1 <目标Git仓库路径>
# 在指定 git 仓库中构建越南宪制文件历史（主分支 = 现行《越南社会主义共和国宪法》2013；1946/1959/1980/1992 历史分支）
...
$TMPDIR = Join-Path $env:TMP "legalize-vn-build-$([System.IO.Path]::GetRandomFileName())"
...
function Build-HistoricalBranches {
    ...
    foreach ($h in $hist) {
        ...
        $text = "# $($h.Display)`n`n> $($h.Note)`n`n$(Build-TOC $body)`n`n$body`n`n---`n`n资料来源：https://en.wikisource.org/wiki/$($h.Title)"
